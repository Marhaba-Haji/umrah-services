...
function SearchForm({
  ...
  return (
    <form className="flex flex-col gap-4 px-3 py-4 md:px-6 md:py-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          ...
          <Select value={searchParams.tripType} onValueChange={(value) => setSearchParams(prev => ({ ...prev, tripType: value }))}>
            <SelectTrigger className="w-full h-11 rounded-lg border border-gray-200 bg-white text-sm font-medium focus:border-emerald-500 focus:ring-emerald-500">
        ...
        <div>
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="outline" className="w-full justify-start h-11 rounded-lg border border-gray-200 bg-white text-sm font-medium focus:border-emerald-500">
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-3">
                    <Button size="sm" variant="outline" className="h-8 w-8 p-0" onClick={() => setSearchParams(prev => ({ ...prev, adults: Math.max(1, prev.adults - 1) }))} disabled={searchParams.adults <= 1}>
                      <Minus className="w-3 h-3" />
                    </Button>
                    <span className="w-8 text-center font-medium">{searchParams.adults}</span>
                    <Button size="sm" variant="outline" className="h-8 w-8 p-0" onClick={() => setSearchParams(prev => ({ ...prev, adults: Math.min(9, prev.adults + 1) }))} disabled={searchParams.adults >= 9}>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-3">
                    <Button size="sm" variant="outline" className="h-8 w-8 p-0" onClick={() => setSearchParams(prev => ({ ...prev, children: Math.max(0, prev.children - 1) }))} disabled={searchParams.children <= 0}>
                      <Minus className="w-3 h-3" />
                    </Button>
                    <span className="w-8 text-center font-medium">{searchParams.children}</span>
                    <Button size="sm" variant="outline" className="h-8 w-8 p-0" onClick={() => setSearchParams(prev => ({ ...prev, children: Math.min(9, prev.children + 1) }))} disabled={searchParams.children >= 9}>
                      <Plus className="w-3 h-3" />
                    </Button>
                  </div>
                </div>
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-3">
                    <Button size="sm" variant="outline" className="h-8 w-8 p-0" onClick={() => setSearchParams(prev => ({ ...prev, infants: Math.max(0, prev.infants - 1) }))} disabled={searchParams.infants <= 0}>
                      <Minus className="w-3 h-3" />
                    </Button>
                    <span className="w-8 text-center font-medium">{searchParams.infants}</span>
                    <Button size="sm" variant="outline" className="h-8 w-8 p-0" onClick={() => setSearchParams(prev => ({ ...prev, infants: Math.min(searchParams.adults, prev.infants + 1) }))} disabled={searchParams.infants >= searchParams.adults}>
                      <Plus className="w-3 h-3" />
                    </Button>
                  </div>
                </div>
      ...
      <div className="grid grid-cols-5 gap-2 items-end">
              ...
              <PopoverContent className="w-[320px] p-0 max-h-72 overflow-auto">
                {fromLoading ? (
                  ...
                  fromSuggestions.map((a, idx) => (
                    <button
                      key={a.code + idx}
                      className="w-full text-left px-4 py-3 hover:bg-emerald-50 focus:bg-emerald-100 focus:outline-none border-b border-gray-100 last:border-0"
                      onClick={() => {
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <span className="font-semibold text-emerald-700">{a.code}</span> - {a.city}, {a.country}
        ...
        <div className="col-span-1 flex justify-center">
          <Button type="button" variant="ghost" size="icon" onClick={swapLocations} className="h-11 w-11 rounded-full border border-gray-200 hover:bg-emerald-50">
        ...
        <div className="col-span-2">
          ...
          >
            <SelectTrigger className="w-full h-11 rounded-lg border-gray-200 focus:border-emerald-500 focus:ring-emerald-500">
        ...
        <div>
          ...
          <Popover open={departureCalendarOpen} onOpenChange={setDepartureCalendarOpen}>
            <PopoverTrigger asChild>
              <Button
                variant="outline"
                className={cn("h-11 w-full justify-start text-left font-normal border-gray-200 focus:border-emerald-500", !searchParams.departureDate && "text-muted-foreground")}
              >
          ...
          <div>
            ...
            <Popover open={returnCalendarOpen} onOpenChange={setReturnCalendarOpen}>
              <PopoverTrigger asChild>
                <Button
                  variant="outline"
                  className={cn("h-11 w-full justify-start text-left font-normal border-gray-200 focus:border-emerald-500", !searchParams.returnDate && "text-muted-foreground")}
                >
                  <CalendarIcon className="mr-2 h-4 w-4" />
      ...
      <div className="flex justify-center mt-4">
        <Button
          ...
          className="h-12 px-8 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white font-semibold text-base flex items-center gap-3 shadow-lg hover:shadow-xl transition-all duration-300"
        >
          {searchLoading ? (
            <>
...
const FlightSearch: React.FC<FlightSearchProps> = ({ onFlightSelect, className, initialPassengers }) => {
  ...
  // Auto-focus management for input
  useEffect(() => {
  ...
  const swapLocations = () => {
    ...
    // Swap the query strings too
    const temp = fromQuery;
    setFromQuery('Jeddah (JED)'); // Since destination is always Saudi cities
  };
          ...
          <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-6 gap-4">
            <Dialog open={searchDialogOpen} onOpenChange={setSearchDialogOpen}>
              <DialogTrigger asChild>
                <Button variant="outline" className="rounded-lg px-6 py-2 font-medium border-emerald-200 text-emerald-700 hover:bg-emerald-50">
                  Modify Search
                </Button>
              </DialogTrigger>
              <DialogContent className="max-w-3xl w-full">
                <div className="mb-4 text-xl font-semibold text-gray-800">Modify Search</div>
                <SearchForm
            ...
            <div className="flex items-center gap-3">
              ...
              <Select value={sortOption} onValueChange={v => setSortOption(v as any)}>
                <SelectTrigger className="w-[140px] h-10 rounded-lg border-gray-200 focus:border-emerald-500 focus:ring-emerald-500">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
          ...
          {loading ? (
            <div className="flex flex-col items-center justify-center py-20">
              <div className="relative flex flex-col items-center">
                ...
                <div className="w-32 h-1 bg-gradient-to-r from-emerald-200 via-emerald-500 to-emerald-200 rounded-full mb-4">
                  <div className="h-full bg-emerald-600 rounded-full animate-pulse"></div>
                </div>
                <span className="text-emerald-700 font-semibold text-lg">Searching for the best flights...</span>
            ...
            <div className="space-y-4">
              {sortedFlights.map((flight) => {
                ...
                return (
                  <Card
                    key={flight.id}
                    className="hover:shadow-lg transition-all duration-300 border border-gray-200 hover:border-emerald-200"
                  >
                    <CardContent className="p-6">
                      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <h4 className="font-bold text-lg text-gray-800">{flight.airline}</h4>
                            <p className="text-gray-600 font-medium">{flight.flightNumber}</p>
                            {flight.aircraft && (
                        ...
                        <div className="flex-1 grid grid-cols-3 gap-4 items-center">
                          <div className="text-center">
                            <div className="font-bold text-xl text-gray-800">{formatTime(flight.departure.at)}</div>
                            <div className="text-sm text-gray-500 font-medium">{flight.departure.iataCode}</div>
                          ...
                          <div className="text-center">
                            <div className="text-sm text-gray-600 font-medium mb-2">{formatDuration(flight.duration)}</div>
                            ...
                            <div className="text-xs text-gray-500">
                              {flight.stops === 0 ? (
                                <Badge variant="secondary" className="bg-green-100 text-green-700">Direct</Badge>
                              ) : (
                          ...
                          <div className="text-center">
                            <div className="font-bold text-xl text-gray-800">{formatTime(flight.arrival.at)}</div>
                            <div className="text-sm text-gray-500 font-medium">{flight.arrival.iataCode}</div>
...
const FlightDetails = ({ offer }: { offer: any }) => {
  ...
  return (
    <Tabs defaultValue="onward" className="w-full">
