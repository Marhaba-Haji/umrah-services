...
const FEATURED_SERVICE_OPTIONS = [
  "Makkah Ziarath",
  "Madinah Ziarath",
  "Taif Ziarath",
  "Badr Ziarath",
  ...
  "Umrah Guide",
  "Jeddah Guide",
  // Optionally, you can add more if needed
];
...
const GuideServicesManager = () => {
  ...
  const fetchGuides = async () => {
    ...
    const { data, error } = await supabase
      .from("guide_services")
      .select(
        "id,guide_name,guide_photo,guide_city,country_code,phone_number,service_type,service_prices,languages,experience,rating,status,description,availability_schedule,qualifications,specializations,services_offered,featured_service",
      );
    if (!error && data) {
      ...
      const transformedGuides = data.map((guide) => ({
        ...guide,
        service_prices:
          (guide.service_prices as { [service: string]: string }) || {},
        availability_schedule: guide.availability_schedule,
        services_offered: guide.services_offered || [],
  ...
  async function getCroppedImg(
    imageSrc: string,
    cropPixels: Record<string, number>,
  ) {
  ...
  const onSubmit = async (data: GuideFormData) => {
    ...
    ).map((label) => SERVICE_TYPE_ENUM_MAP[label] || label);
    // Languages as array
    const languagesArr =
    ...
    const pricesObj =
      servicePrices && Object.keys(servicePrices).length > 0
        ? servicePrices
        : null;
    ...
    if (
      ...
      toast({
        ...
        description:
          "Guide Name, Guide City, at least one Service Type, and Guide Photo are required.",
        variant: "destructive",
      });
      return;
    }
    ...
    const newGuide = {
      ...
      service_prices: pricesObj
        ? Object.fromEntries(
            Object.entries(pricesObj).map(([label, price]) => [
              SERVICE_TYPE_ENUM_MAP[label] || label,
              price,
            ]),
          )
        : null,
  ...
  const handleEdit = (guide: GuideService) => {
    ...
    form.reset({
      ...
      serviceType: (guide.service_type || []).map(
        (enumVal) =>
          Object.keys(SERVICE_TYPE_ENUM_MAP).find(
            (key) => SERVICE_TYPE_ENUM_MAP[key] === enumVal,
          ) || enumVal,
      ),
      servicePrices: guide.service_prices
        ? Object.fromEntries(
            Object.entries(guide.service_prices).map(([key, price]) => [
              Object.keys(SERVICE_TYPE_ENUM_MAP).find(
                (label) => SERVICE_TYPE_ENUM_MAP[label] === key,
              ) || key,
              price,
            ]),
          )
      ...
      qualifications: guide.qualifications
        ? guide.qualifications.join(", ")
        : "",
      specializations: guide.specializations
        ? guide.specializations.join(", ")
        : "",
      servicesOffered: guide.services_offered || [],
      featuredService: guide.featured_service || "",
    });
    setCountryCode(
      COUNTRY_CODES.find((c) => c.code === guide.country_code) ||
        COUNTRY_CODES[0],
    );
      ...
      <div className="flex justify-between items-center">
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>
                {editingGuide ? "Edit Guide Service" : "Add New Guide Service"}
              </DialogTitle>
            </DialogHeader>
            <DialogDescription>
              Fill in the details for the guide service. All fields marked * are
              required.
            </DialogDescription>
            <Form {...form}>
              <form
              ...
              >
                  ...
                  <FormField
                    ...
                    render={({ field }) => {
                      ...
                      const selectedCountry =
                        COUNTRY_CODES.find((c) => c.code === field.value) ||
                        COUNTRY_CODES[0];
                        ...
                        <FormItem>
                          ...
                          <div className="relative">
                            <Input
                              ...
                              style={{
                                backgroundColor: countryDropdownOpen
                                  ? undefined
                                  : "#f9fafb",
                              }}
                            />
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>
                        Availability Schedule (JSON or description)
                      </FormLabel>
                      <FormControl>
                        <Textarea
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input
                          placeholder="Certified, 10+ years experience"
                          {...field}
                        />
                      </FormControl>
                      <FormMessage />
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      >
                        <FormControl>
                          <SelectTrigger>
                            <SelectValue placeholder="Select featured service" />
                          </SelectTrigger>
                        </FormControl>
                        <SelectContent>
                          {FEATURED_SERVICE_OPTIONS.map((option) => (
                    ...
                    <tr key={guide.id} className="border-b hover:bg-gray-50">
                      ...
                      <td className="p-2">
                        {Array.isArray(guide.service_type)
                          ? guide.service_type.map((type, idx) => (
                              <span key={guide.id + "-" + type + "-" + idx}>
                                {type}
                                {idx < guide.service_type.length - 1
                                  ? ", "
                                  : ""}
                              </span>
                            ))
                      ...
                      <td className="p-2">
                        <Badge
                          variant={
                            guide.status === "Active" ? "default" : "secondary"
                          }
                        >
                          {guide.status}
                        </Badge>
          ...
          {viewingGuide && (
            <div className="space-y-2">
              <div className="flex items-center gap-4">
                <img
                  src={viewingGuide.guide_photo}
                  alt={viewingGuide.guide_name}
                  className="w-16 h-16 rounded-full object-cover border"
                />
                <div>
                  <div className="font-bold text-lg">
                    {viewingGuide.guide_name}
                  </div>
                  <div className="text-gray-500 text-sm">
                    {viewingGuide.country_code}
                    {viewingGuide.phone_number}
                  </div>
                </div>
              </div>
              <div>
                <b>City:</b> {viewingGuide.guide_city}
              </div>
              <div>
                <b>Service Types:</b>{" "}
                {Array.isArray(viewingGuide.service_type)
                  ? viewingGuide.service_type.map((type, idx) => (
                      <span key={viewingGuide.id + "-" + type + "-" + idx}>
                        {type}
                        {idx < viewingGuide.service_type.length - 1 ? ", " : ""}
                      </span>
                    ))
                  : viewingGuide.service_type}
              </div>
              <div>
                <b>Languages:</b> {viewingGuide.languages?.join(", ")}
              </div>
              <div>
                <b>Experience:</b> {viewingGuide.experience}
              </div>
              <div>
                <b>Rating:</b> {viewingGuide.rating}
              </div>
              <div>
                <b>Status:</b> {viewingGuide.status}
              </div>
              <div>
                <b>Description:</b> {viewingGuide.description}
              </div>
              <div>
                <b>Qualifications:</b>{" "}
                {viewingGuide.qualifications?.map((q, idx) => (
                  <span key={viewingGuide.id + "-" + q + "-" + idx}>
                    {q}
                    {idx < viewingGuide.qualifications.length - 1 ? ", " : ""}
                  </span>
                ))}
              </div>
              <div>
                <b>Specializations:</b>{" "}
                {viewingGuide.specializations?.map((s, idx) => (
                  <span key={viewingGuide.id + "-" + s + "-" + idx}>
                    {s}
                    {idx < viewingGuide.specializations.length - 1 ? ", " : ""}
                  </span>
                ))}
              </div>
              <div>
                <b>Availability:</b> {viewingGuide.availability_schedule}
              </div>
              <div>
                <b>Service Prices:</b>{" "}
                <pre className="bg-gray-100 rounded p-2 text-xs overflow-x-auto">
                  {JSON.stringify(viewingGuide.service_prices, null, 2)}
                </pre>
              </div>
            </div>
          )}
