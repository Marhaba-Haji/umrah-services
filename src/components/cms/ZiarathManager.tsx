...
const ZiarathManager = () => {
  ...
  const [editingZiarath, setEditingZiarath] = useState<ZiarathService | null>(
    null,
  );
  const [viewingZiarath, setViewingZiarath] = useState<ZiarathService | null>(
    null,
  );
  ...
  const fetchZiaraths = async () => {
    ...
    const { data, error } = await supabase
      ...
      .select(
        "id,title,location,duration,price,description,ziarath_type,max_participants,inclusions,significance,historical_importance,best_time,images,status,vehicle_prices",
      );
    if (!error && data) {
      setZiaraths(data);
    }
  ...
  const onSubmit = async (data: unknown) => {
    const newZiarath = {
      ...
      inclusions: data.inclusions
        ? data.inclusions.split(",").map((i: string) => i.trim())
        : [],
  ...
  const handleDelete = async (id: string) => {
    ...
    if (
      !window.confirm(
        "Are you sure you want to delete this ziarath service? This action cannot be undone.",
      )
    )
      return;
  ...
  return (
    <div className="space-y-6">
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>
                {editingZiarath
                  ? "Edit Ziarath Service"
                  : "Add New Ziarath Service"}
              </DialogTitle>
            </DialogHeader>
            <DialogDescription>
              Fill in the details for the ziarath service. All fields marked *
              are required.
            </DialogDescription>
            <Form {...form}>
              <form
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Textarea
                            placeholder="Enter historical importance"
                            {...field}
                          />
                        </FormControl>
                        <FormMessage />
                      ...
                      <td className="p-2">
                        <div className="flex space-x-1">
                          <Button
                            size="sm"
                            variant="outline"
                            onClick={() => setViewingZiarath(ziarath)}
                          >
                          ...
                          <Button
                            size="sm"
                            variant="outline"
                            onClick={() => handleDelete(ziarath.id)}
                          >
      ...
      <Dialog
        open={!!viewingZiarath}
        onOpenChange={(open) => {
          if (!open) setViewingZiarath(null);
        }}
      >
        <DialogContent className="max-w-lg">
          ...
          {viewingZiarath && (
            <div className="space-y-2">
              <div>
                <b>Title:</b> {viewingZiarath.title}
              </div>
              <div>
                <b>Location:</b> {viewingZiarath.location}
              </div>
              <div>
                <b>Duration:</b> {viewingZiarath.duration}
              </div>
              <div>
                <b>Price:</b> ${viewingZiarath.price}
              </div>
              <div>
                <b>Description:</b> {viewingZiarath.description}
              </div>
              <div>
                <b>Ziarath Type:</b> {viewingZiarath.ziarath_type}
              </div>
              <div>
                <b>Max Participants:</b> {viewingZiarath.max_participants}
              </div>
              <div>
                <b>Inclusions:</b> {viewingZiarath.inclusions?.join(", ")}
              </div>
              <div>
                <b>Significance:</b> {viewingZiarath.significance}
              </div>
              <div>
                <b>Historical Importance:</b>{" "}
                {viewingZiarath.historical_importance}
              </div>
              <div>
                <b>Best Time to Visit:</b> {viewingZiarath.best_time}
              </div>
              <div>
                <b>Images:</b>{" "}
                {viewingZiarath.images?.map((img, idx) => (
                  <img
                    key={idx}
                    src={img}
                    alt={`Ziarath image ${idx + 1}`}
                    className="w-24 h-24 object-cover rounded mr-2 inline-block"
                  />
                ))}
              </div>
              <div>
                <b>Status:</b> {viewingZiarath.status}
              </div>
            </div>
          )}
        </DialogContent>
