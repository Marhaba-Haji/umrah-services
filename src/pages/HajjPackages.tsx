...
const defaultPriceRange = [200000, 800000];
...
const HajjPackages = () => {
  ...
  useEffect(() => {
    ...
    if (filters.duration) {
      pkgs = pkgs.filter((pkg) => {
        if (filters.duration === "short") return getDays(pkg.duration) < 18;
        if (filters.duration === "medium")
          return getDays(pkg.duration) >= 18 && getDays(pkg.duration) <= 25;
        if (filters.duration === "long") return getDays(pkg.duration) > 25;
        return true;
      });
    }
          ...
          <div className="grid grid-cols-2 gap-2">
            {maktabCategories.map((cat) => (
              <div key={cat} className="flex items-center space-x-2">
                ...
                />
                <Label htmlFor={`maktab-${cat}`} className="text-sm">Category {cat}</Label>
              </div>
            ))}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="px-2">
            ...
            <div className="flex justify-between text-xs text-gray-600 mt-2">
              <span>{(() => {
                const { symbol, value } = convertFromINR(filters.price[0], currency);
                return `${symbol}${value.toLocaleString()}`;
              })()}</span>
              <span>{(() => {
                const { symbol, value } = convertFromINR(filters.price[1], currency);
                return `${symbol}${value.toLocaleString()}`;
              })()}</span>
            </div>
          </div>
          ...
          <div className="space-y-2">
            {classes.map((c) => (
              <div key={c} className="flex items-center space-x-2">
                ...
                />
                <Label htmlFor={`class-${c}`} className="text-sm">{c}</Label>
              </div>
            ))}
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-emerald-50">
      ...
      {/* Hero Section */}
      <section className="py-8 bg-gradient-to-r from-emerald-600 via-teal-600 to-blue-600 relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 text-center relative z-10">
          ...
          <p className="text-lg md:text-xl text-white/90 max-w-3xl mx-auto leading-relaxed">
            Embark on the spiritual journey of a lifetime with our carefully curated Hajj packages, 
            designed to provide comfort, guidance, and peace of mind throughout your pilgrimage.
          </p>
        </div>
      </section>
            ...
            <div className="flex-1">
              ...
              {isLoading ? (
                ...
                <div className="text-center py-12">
                  <div className="text-gray-500 text-lg mb-2">No Hajj packages found</div>
