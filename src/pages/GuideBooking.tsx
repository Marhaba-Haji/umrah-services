...
// Service type value/label mapping (should match GuideServicesManager)
const SERVICE_TYPE_ENUM_MAP = {
  ...
  "Jeddah Guide": "jeddah_guide",
  "Meet & Greet": "meet_and_greet",
};
const SERVICE_TYPE_LABEL_MAP = Object.fromEntries(
  Object.entries(SERVICE_TYPE_ENUM_MAP).map(([label, value]) => [value, label]),
);
const SERVICE_TYPES = Object.entries(SERVICE_TYPE_ENUM_MAP).map(
...
const GuideBooking = () => {
  ...
  const getGuideServicePrice = (guide: GuideService, serviceType: string) => {
    // Map internal value to label for lookup
    const label = SERVICE_TYPE_LABEL_MAP[serviceType] || serviceType;
    if (!guide.service_prices || !label) return 0;
    const price = guide.service_prices[label];
    return price
      ? typeof price === "string"
        ? parseFloat(price)
        : Number(price)
  ...
  const getGuideSubtotal = (
    guide: GuideService,
    details: GuideDetailsState,
  ) => {
    ...
    try {
      ...
      if (data) {
        ...
        const transformedGuides = data.map((guide) => ({
          ...guide,
          service_type: Array.isArray(guide.service_type)
            ? guide.service_type
            : [guide.service_type].filter(Boolean),
        }));
  ...
  const filteredGuides = guides
    .filter((guide) => {
      ...
      if (
        ...
        !(guide.languages || []).some((l) => filterLanguage.includes(l))
      )
        return false;
      ...
      if (filterExperience !== "any") {
        ...
        if (filterExperience === "0-1" && !(years >= 0 && years <= 1))
          return false;
        if (filterExperience === "2-5" && !(years >= 2 && years <= 5))
          return false;
        if (filterExperience === "6-10" && !(years >= 6 && years <= 10))
          return false;
  ...
  // Use only SERVICE_TYPES for filter options
  const filteredServiceTypes = SERVICE_TYPES;
  ...
  const handleBooking = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: "Success",
        description:
          "Your guide booking request(s) have been submitted! We'll contact you within 24 hours to confirm details.",
      });
      // Reset form
      setBookingForm({
        name: "",
          ...
          <div className="flex items-center space-x-4">
            {[1, 2, 3].map((step) => (
              <div key={step} className="flex items-center">
                ...
                {step < 3 && (
                  <div
                    className={`w-10 h-0.5 ${currentStep > step ? "bg-emerald-600" : "bg-gray-200"}`}
                  />
                )}
              </div>
            ))}
            ...
            <Card className="sticky top-4 mb-4">
              ...
              <CardContent className="space-y-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {allLanguages.map((lang) => (
                      <Button
                        ...
                        size="sm"
                        variant={
                          filterLanguage.includes(lang) ? "default" : "outline"
                        }
                        onClick={() =>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {filteredServiceTypes.map((typeObj) => (
                      <Button
                        key={typeObj.value}
                        size="sm"
                        variant={
                          filterServiceType.includes(typeObj.value)
                            ? "default"
                            : "outline"
                        }
                        onClick={() =>
                          setFilterServiceType((prev) =>
                            prev.includes(typeObj.value)
                              ? prev.filter((t) => t !== typeObj.value)
                              : [...prev, typeObj.value],
                          )
                        }
                  ...
                  <CardContent className="p-6">
                    <div className="bg-emerald-50 p-4 rounded-lg mb-6">
                      <h4 className="font-semibold text-emerald-800 mb-2">
                        {selectedGuides[0].guide_name}
                      </h4>
                      <p className="text-emerald-700 text-sm">
                        {selectedGuides[0].guide_city} •{" "}
                        {selectedGuides[0].experience}
                      </p>
                    </div>
                    ...
                    <div className="space-y-6">
                      <div>
                        ...
                        >
                          ...
                          <SelectContent>
                            <SelectItem value="ziarath">
                              Ziarath Tour Guide
                            </SelectItem>
                            <SelectItem value="city_tour">
                              City Tour Guide
                            </SelectItem>
                            <SelectItem value="religious_guidance">
                              Religious Guidance
                            </SelectItem>
                            <SelectItem value="airport_assistance">
                              Airport Assistance
                            </SelectItem>
                          </SelectContent>
                        </Select>
                ...
                {selectedGuides.length > 0 ? (
                  <div className="space-y-4">
                    {selectedGuides.map((guide) => {
                      ...
                      const availableTypes = Array.isArray(guide.service_type)
                        ? guide.service_type
                        : guide.service_type
                          ? [guide.service_type]
                          : [];
                      const price = getGuideServicePrice(
                        guide,
                        details.serviceType,
                      );
                      ...
                      return (
                        <div
                          key={guide.id}
                          className="bg-emerald-50 rounded p-3 mb-2"
                        >
                          <div className="flex items-center justify-between mb-1">
                            ...
                            >
                              ...
                              <SelectContent>
                                {availableTypes.map((type) => (
                                  ...
                                  >
                                    {SERVICE_TYPE_LABEL_MAP[type] || type}
                                  </SelectItem>
                                ))}
                              </SelectContent>
                            </Select>
