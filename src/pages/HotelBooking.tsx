...
import { format, addDays, differenceInCalendarDays, isBefore, isAfter, format as formatDate } from 'date-fns';
...
const HotelBooking = () => {
  ...
  const fetchAndFilterHotels = async () => {
    ...
    if (
      ...
      (activeFilters.priceRange[0] > defaultPriceRange[0] || activeFilters.priceRange[1] < defaultPriceRange[1])
    ) {
    ...
    if (
      ...
      (activeFilters.distanceRange[0] > defaultDistanceRange[0] || activeFilters.distanceRange[1] < defaultDistanceRange[1])
    ) {
      const distanceColumn = activeFilters.city === 'makkah' ? 'distance_from_haram' : 'distance_from_masjid_e_nabawi';
    ...
    const sortOptions = {
      ...
      'distance-asc': { column: activeFilters.city === 'makkah' ? 'distance_from_haram' : 'distance_from_masjid_e_nabawi', ascending: true },
    };
  ...
  useEffect(() => {
    if (enquiryForm.checkIn && enquiryForm.checkOut) {
      const nights = differenceInCalendarDays(new Date(enquiryForm.checkOut), new Date(enquiryForm.checkIn));
  ...
  const handleRoomGuestChange = (idx: number, value: number) => {
    const newRooms = enquiryForm.rooms.map((room, i) => i === idx ? { guests: Math.max(1, Math.min(4, value)) } : room);
    setEnquiryForm({ ...enquiryForm, rooms: newRooms });
  };

  const addRoom = () => {
    if (enquiryForm.rooms.length < 5) {
      setEnquiryForm({ ...enquiryForm, rooms: [...enquiryForm.rooms, { guests: 1 }] });
    }
  };

  const removeRoom = (idx: number) => {
    if (enquiryForm.rooms.length > 1) {
      setEnquiryForm({ ...enquiryForm, rooms: enquiryForm.rooms.filter((_, i) => i !== idx) });
    }
  };
      ...
      const { data, error } = await supabase.from('hotel_enquiries').insert([
        {
          ...
          check_out: enquiryForm.checkOut ? new Date(enquiryForm.checkOut) : null,
      ...
      <Card className="overflow-hidden hover:shadow-lg transition-shadow duration-200">
        <div className="md:flex">
          <div className="md:w-1/3">
            <img 
              src={hotel.images && hotel.images.length > 0 ? hotel.images[0] : 'https://images.unsplash.com/photo-1551882547-ff40c63fe5fa?w=400&h=250&fit=crop'} 
              alt={hotel.name}
          ...
          <div className="md:w-2/3 p-6">
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              {hotel.amenities && hotel.amenities.slice(0, 4).map((amenity: string, index: number) => (
            ...
            <div className="flex justify-between items-center">
              ...
              <Button className="bg-emerald-600 hover:bg-emerald-700 text-white" onClick={() => openEnquiry(hotel)}>
                Enquire Now
              </Button>
            </div>
          </div>
            ...
            <div className="lg:col-span-3 flex flex-col items-center">
              ...
              <div className="flex justify-between items-center w-full mb-4">
                <div className="flex items-center gap-2">
                  {activeFilters.city === 'makkah' && <span className="text-2xl">🕋</span>}
                  {activeFilters.city === 'madinah' && <span className="text-2xl">🕌</span>}
                ...
                <Select value={sortBy} onValueChange={setSortBy}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="distance-asc" disabled={activeFilters.city === 'all'}>
                      Distance: Closest
                    </SelectItem>
                  </SelectContent>
                </Select>
        ...
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={handleEnquirySubmit} className="space-y-4">
            <div>
              ...
              <Input value={enquiryHotel?.name || ''} readOnly className="bg-gray-100" />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium mb-1">Check-in</label>
                <Input name="checkIn" type="date" value={enquiryForm.checkIn} onChange={handleEnquiryChange} required min={today} />
                {enquiryForm.checkIn && (
              ...
              <div>
                ...
                <Input name="checkOut" type="date" value={enquiryForm.checkOut} onChange={handleEnquiryChange} required min={enquiryForm.checkIn ? formatDate(addDays(new Date(enquiryForm.checkIn), 1), 'yyyy-MM-dd') : tomorrow} />
            ...
            {nightCount !== null && (
              <div className="text-sm text-emerald-700 font-medium mb-2">Nights: {nightCount}</div>
            )}
            <div>
                ...
                {enquiryForm.rooms.map((room, idx) => (
                  <div key={idx} className="flex items-center gap-2">
                    ...
                    <Input
                      ...
                      max={4}
                      value={room.guests}
                      onChange={e => handleRoomGuestChange(idx, parseInt(e.target.value) || 1)}
                      className="w-20"
                    />
                    <span className="text-xs text-gray-600">guests</span>
                    {enquiryForm.rooms.length > 1 && (
                      <Button type="button" size="sm" variant="outline" onClick={() => removeRoom(idx)}>-</Button>
                    )}
                  </div>
                ))}
                <Button type="button" size="sm" variant="outline" onClick={addRoom} disabled={enquiryForm.rooms.length >= 5}>+ Add Room</Button>
              </div>
            </div>
            <div>
            ...
            <div>
              ...
              <Input name="email" type="email" value={enquiryForm.email} onChange={handleEnquiryChange} required />
            </div>
            <div>
                ...
                <div className="relative w-40" ref={countryDropdownRef}>
                  ...
                  >
                    <span className="mr-2 text-lg">
                      {COUNTRY_CODES.find(c => c.code === enquiryForm.countryCode)?.flag}
                    </span>
                    <span className="mr-1 text-sm font-medium">
                      {COUNTRY_CODES.find(c => c.code === enquiryForm.countryCode)?.code}
                    </span>
                    <span className="text-xs text-gray-500 truncate">
                      {COUNTRY_CODES.find(c => c.code === enquiryForm.countryCode)?.name}
                    </span>
                    ...
                    <div className="absolute left-0 mt-1 w-full bg-white border rounded shadow-lg z-20 max-h-48 overflow-y-auto">
                      ...
                      {filteredCountryCodes.map(c => (
                        <div
                          ...
                          onMouseDown={() => { handleCountryCodeChange(c.code); setCountryDropdownOpen(false); setCountrySearch(''); }}
                        >
            ...
            <div>
              ...
              <Textarea name="message" value={enquiryForm.message} onChange={handleEnquiryChange} rows={3} required />
            </div>
            <div className="flex justify-end gap-2 pt-2">
              <Button type="button" variant="outline" onClick={closeEnquiry} disabled={enquiryLoading}>Cancel</Button>
              <Button type="submit" disabled={enquiryLoading}>
                {enquiryLoading ? (
                  <span className="flex items-center gap-2"><svg className="animate-spin h-4 w-4 mr-1 text-white" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" fill="none" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v4a4 4 0 00-4 4H4z" /></svg>Submitting...</span>
                ) : (
                  'Submit Enquiry'
                )}
              </Button>
